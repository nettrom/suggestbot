...
class SuggestBot:
    ...
    def setConfig(self, key=None, value=None):
        ...
        if not key:
            sys.stderr.write("SBot Warning: setConfig called without a key.\n");
            return False;
    ...
    def getConfig(self, key=None):
        ...
        if not key:
            sys.stderr.write("SBot Warning: getConfig called without a key.\n");
    ...
    def getRecs(self, username="", userGroup="suggest", itemEnd=True,
        ...
        try :
            ...
            sys.stderr.write(u"Error: something went wrong when trying to get suggestions:\n");
            sys.stderr.write(u"Error: {error}\n".format(error=e));
    ...
    def postRecommendations(self, username=u"", recMsg=None,
        ...
        if not username or not recMsg:
            sys.stderr.write("SuggestBot Warning: Unable to post recs, username or recommendation not supplied.\n");
            return False;
        ...
        try:
            ...
            sys.stderr.write(u"SuggestBot Warning: Destination page {title} doesn't exist, will be created.\n".format(title=destPage.title()).encode('utf-8'));
            pageSource = "";
        ...
        # else: update it, and ask to watch it too
            ...
            try:
                ...
                sys.stderr.write(u"SuggestBot Error: Posting recommendations to {title} failed, edit conflict.\n".format(title=destPage.title()).encode('utf-8'));
                return False;
            except pywikibot.exceptions.PageNotSaved as e:
                sys.stderr.write(u"SuggestBot Error: Failed posting recommendations to {title}.\nError: {emsg}\n".format(title=destPage.title(), emsg=e).encode('utf-8'));
                return False;
        # done
        return True;
    ...
    def getPageLinks(self, pageTitles=None, namespaces=None):
        ...
        if not pageTitles:
            sys.stderr.write("SBot Warning: getPageLinks called with no page titles.\n");
            return None;
        ...
        while i < max_ind:
            ...
            if self.config.getConfig('VERBOSE') == True:
                print "DEBUG: Getting links for {n} titles through the API.".format(n=len(titles));
            ...
            while not allDone:
                ...
                if 'query-continue' in json_data:
                    ...
                    if self.config.getConfig('VERBOSE'):
                        sys.stderr.write("SBot debug: API continue at {cont}\n".format(cont=cont_value));
                else:
                    allDone = True;
    ...
    def processRegulars(self):
        ...
        if ord(row['daily_running']):
            sys.stderr.write(u"Info: A SuggestBot job is already posting to users on {0}-WP, exiting!\n".format(lang).encode('utf-8'));
            return True;
        ...
        if row and row['last_rec']:
            ...
            if timeSinceLastRun.days < 0:
                sys.stderr.write("Error: Time since last set of recs posted is negative, aborting!\n");
                return False;
        else:
        ...
        if self.verbose:
            sys.stderr.write("Info: looking %d seconds ahead for due recs.\n" % (lookaheadTime,));
        ...
        while not done:
            ...
            if design:
                ...
                try:
                    ...
                    pass;
                if self.verbose:
                    sys.stderr.write(u"Info: found design {0} with template {1}\n".format(design, recTemplate).encode('utf-8'));
        ...
        if self.verbose:
            sys.stderr.write("SBot Info: Checked regulars, found %d users to post to.\n" % (len(userQueue),));
        ...
        for user in userQueue:
            ...
            if self.verbose:
                sys.stderr.write("SBot Info: now getting recs for user '%s'.\n" % (user['username'].encode('utf-8'),));
            ...
            if success:
                ...
                sys.stderr.write("SBot Info: Posted recs to %s.\n" \
                                     % (user['username'].encode('utf-8'),));
